...
def robotToLegTF(vect):
    return np.array([-vect[1],vect[0],vect[2]])

class QuadLeg:
    """ Robot leg """
    def __init__(self, hipVectorA, hipVectorB, isRightSide = False):
        self.isRightSide = isRightSide
        self.hipA_angle = 0.0
        self.hipB_angle = 0.0
        self.hipR_angle = 0.0
        self.hipAPoint = robotToLegTF(hipVectorA)
        self.hipBPoint = robotToLegTF(hipVectorB)
    ...
    def getEffectorSolution(self, groundNormal, groundPoint):
        anklePoint = robotToLegTF(groundNormal) * qS.FOOT_RADIUS + robotToLegTF(groundPoint)
        tiltProjAnkle = self.hipAPoint - anklePoint
        tiltProjAnkle[1] = 0

        # Find Femur Tilt Poses
        hipR_angle = 0
        femur_tilt = 0
        femurVect = np.zeros(3)
        if(self.isRightSide):
